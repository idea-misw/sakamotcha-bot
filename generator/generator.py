...
class Generator:
    ...
    def load(self, file_path=None):
        ...
        self.ngram_size, self.originals, self.chain = load_data
    ...
    def learn(self, text):
        ...
        sequence = [self.bos_token] * (self.ngram_size - 1) \
            + self.tokenizer.tokenize(text) + [self.eos_token]
    ...
    def generate(self):
        for i in range(self.trial_num):
            prefix = (self.bos_token,) * (self.ngram_size - 1)
            sequence = []

            token = random.choice(self.chain[prefix])
            while token != self.eos_token:
